...
resultados_ive <- read.csv("C:/Users/socan/Documents/EANT/Social_Data_Analitcys/Proyecto_final/5_0Entrega.Shiny/resultados_ive.csv") # Dataset general
prov_geo <- st_read("C:/Users/socan/Documents/EANT/Social_Data_Analitcys/Proyecto_final/5_0Entrega.Shiny/prov_geo.shp") # Geometrias de Provincias
resultados_ive_geo <- prov_geo %>% left_join(resultados_ive) # Dataset para solapa "Por provincia"
    ...
    tabsetPanel(
        ...
        tabPanel('Equipo',
                 mainPanel(
                   ...
                   br(),
                   h4("Carmen Tello: ",
                      tags$a(href="https://twitter.com/tarmencello?s=20",
                             "Twitter")),
                   br(),
                   h2(strong("Curso:")),
...
server <- function(input, output) {
  ...
  output$download_data1 <- downloadHandler(
    ...
    content = function(file) {
      ...
      write_xlsx(filt_votacion[,3:11], file)
    }
  )
    ...
    output$download_data2 <- downloadHandler(
      ...
      content = function(file) {
        ...
        write_xlsx(filt_orienpol[,3:11], file)
      }
    )
    ...
    output$download_data3 <- downloadHandler(
      ...
      content = function(file) {
        ...
        write_xlsx(filt_prov[,c(1,4,5,6,7,8,9,10,11)], file)
      }
    )
    ...
    output$download_data5 <- downloadHandler(
      ...
      content = function(file) {
        ...
        write_xlsx(filt_edad[,3:11], file)
      }
    )
